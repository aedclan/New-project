# Personal Hub 当前开发路线

更新时间：2026-06-27

之前的功能扩展路线暂时停止。当前项目优先进入“数据安全与部署自动化”阶段，目标是让网站可以稳定长期运行在 VPS 上，数据可备份、可恢复、可回滚、可持续更新。

## 当前主线

1. 数据安全
2. 部署自动化

## 第一阶段：数据安全

### 目标

保证用户数据不会因为浏览器缓存、Docker 重建、误操作、服务器更新或账号混乱而丢失。

### 已落实内容

1. 真实登录基础
   - 使用服务器账号登录。
   - 使用 HttpOnly Cookie 保存登录状态。
   - 用户密码使用哈希存储。
   - 管理员和普通用户区分权限。

2. 多用户数据隔离
   - 每个用户拥有独立数据文件。
   - 后端根据当前登录用户读取对应数据。
   - 普通用户不能访问其他用户数据。

3. 数据持久化
   - Docker 数据卷保存 `/app/data`。
   - 认证数据库保存到 `/app/data/personal-hub.sqlite`。
   - 用户数据保存到 `/app/data/users/user-用户ID.json`。

4. 备份增强
   - `npm run backup:data` 支持手动备份。
   - 自动备份覆盖认证数据库、旧版全局数据、所有用户数据。
   - 支持配置备份目录、保留数量、备份周期。

5. 恢复工具
   - `npm run backup:list` 查看备份。
   - `npm run restore:data` 恢复数据。
   - 恢复操作必须显式增加 `--yes`，避免误覆盖。

6. 生产配置检查
   - `npm run check:production` 检查管理员账号、强密码、注册安全和备份配置。

### 下一步可继续加强

1. 在设置页面增加备份列表查看。
2. 在设置页面增加手动备份按钮。
3. 在设置页面增加备份恢复入口。
4. 增加导入 JSON 前的数据预览和二次确认。
5. 增加账号修改密码功能。
6. 增加管理员查看用户数据文件状态。

## 第二阶段：部署自动化

### 目标

让本地开发、GitHub 推送、VPS 拉取、Docker 重建、健康检查、失败回滚形成固定流程。

### 已落实内容

1. VPS 一键部署脚本
   - 文件：`scripts/deploy-vps.sh`
   - 作用：
     - 加载 `.env`；
     - 检查生产配置；
     - 运行项目检查；
     - 部署前备份；
     - 记录当前 Git 版本；
     - 拉取 GitHub 最新代码；
     - 重建 Docker；
     - 检查健康接口；
     - 检查域名访问。

2. VPS 回滚脚本
   - 文件：`scripts/rollback-vps.sh`
   - 作用：
     - 回滚到上一次部署前记录的版本；
     - 或回滚到指定 commit；
     - 回滚前尽量先备份；
     - 重建 Docker 并检查健康接口。

3. 部署文档
   - 文件：`DATA_SECURITY_AND_DEPLOYMENT.md`
   - 内容：
     - 生产环境变量；
     - 备份和恢复；
     - 一键部署；
     - 回滚；
     - 本地到 GitHub 到 VPS 的完整流程；
     - 上线前检查清单。

### 标准更新流程

本地：

```powershell
npm run check
git status
git add .
git commit -m "feat: 本次更新说明"
git push origin main
```

VPS：

```bash
cd /opt/personal-hub/New-project
./scripts/deploy-vps.sh
```

异常回滚：

```bash
cd /opt/personal-hub/New-project
./scripts/rollback-vps.sh
```

### 下一步可继续加强

1. 增加 GitHub Actions 自动检查。
2. 增加部署前自动创建 Git tag。
3. 增加部署失败自动回滚。
4. 增加容器日志导出。
5. 增加定期备份下载到本地或对象存储。

## 当前优先验收

1. `npm run check` 通过。
2. `npm run backup:data` 能生成备份。
3. `npm run backup:list` 能查看备份。
4. VPS `.env` 配置真实管理员账号和强密码。
5. `npm run check:production` 通过。
6. `./scripts/deploy-vps.sh` 可以完成更新。
7. `./scripts/rollback-vps.sh` 可以回滚。
8. `https://www.aedclan.com` 正常访问。
